...
const DisplayMetadata = () => {
            ...
            renderCell: (params) => {
                return (
                    <div style={{
                        ...
                        {params.row.logo && <img src={params.row.logo} style={{
                            height: "45px",
                            width: "45px",
                            borderRadius: "50%",
                            objectFit: "cover"
            ...
            renderCell: (params) => {
                return (
                    <div style={{
                        ...
                        <Link to={`/metadata/${params.row._id}`} style={{
                            color: "#green"
                        }}>
                            <EditOutlined />
                        </Link>
    ...
    const onRowsSelectionHandler = (ids) => {
        const selectedRowsData = ids.map((id) => metadata.find((row) => row._id === id));
    ...
    const deleteHandler = () => {
        selectedRows.forEach((item) => {
            ...
            deleteMetadata(token, item._id).then((res) => {
    ...
    return (
        <div className='container-fluid mt-5'>
            <div style={{ background: " #ffffff", color: "#525", position: "relative", padding: "5px 15px", display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "0.9rem" }}>
                <span style={{ fontWeight: "bolder", fontSize: "1.7rem" }}>Metadata</span>

                <span style={{ fontWeight: "bolder", fontSize: "1.5rem" }}>Add Metadata
                    <Link to={`/metadata`}>
                        <IconButton size='lg'>
                            <Add />
                        </IconButton>
